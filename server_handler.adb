...
package body Server_Handler is
    ...
    procedure Server (From: in LLU.End_Point_Type;
        ...
        Admit            : Boolean := True;
        Found            : Boolean := False;
        Del_Active_Client: Boolean := False;
    ...
    begin
        ...
        case Mess is
            ...
            when CM.Init =>
		        ...
		        -- Comprobar Nick --> True indica que al ser mensaje Init solo debo comprobar Nick
		        Found := MT.Check_Nick(Nick, Client_EP_Handler, True);
		        
		        LLU.Reset(P_Buffer.all);             
                -- Si el nick es server o encuentra el mismo nick en el Mapa de Clientes Activos
                if Found then
                    -- Cliente Rechazado
                    raise Nick_Error;
		        ...
		        Boolean'Output(P_Buffer, Admit); -- Admit inicializado a True
            ...
            when CM.Writer =>
		        ...
		        -- Comprobar Nick y EP --> False indica que al ser mensaje Writer debo comprobar tambien el EP
		        Found := MT.Check_Nick(Nick, Client_EP_Handler, False);
            ...
            when CM.Logout =>
		        ...
		        -- Comprobar Nick y Ep--> False indica que al ser mensaje Logout debo comprobar tambien el EP
		        Found := MT.Check_Nick(Nick, Client_EP_Handler, False);
